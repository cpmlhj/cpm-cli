...
const IGNORE_GLOB_PATH = ['node_modules/**', 'public/**', 'src/**']
...
class InitCommand extends Command {
	...
	async getProjectInfo() {
		...
		const initPrompt = [
			{
				...
				validate: function (v) {
					...
					setTimeout(() => {
						...
						if (!project_name_match.test(v))
							return done('请输入正确的名称')
						done(null, true)
					}, 0)
				}
			},
		...
		if (project.projectName) {
			project.className = require('kebab-case')(
				project.projectName
			).replace(/^-/, '')
	...
	async downloadTemplate() {
		...
		const templateInfo = this.tempaltes.find(
			(tem) => tem.npmName === templateName
		)
		// 获取模板缓存目录
	...
	checkCommand(code) {
		...
		throw new Error(
			`npm包<${name}>: 执行的命令${code}为非法命令，不在可执行的白名单中`
		)
	}
