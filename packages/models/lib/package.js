...
class Package {
	...
	async prepare() {
		...
		if (this.packageVersion === 'latest') {
			this.packageVersion = await getLatestNpmVersion({
				npmName: this.packageName
			})
		}
	}
	...
	async install(version) {
		return npminstall({
			...
			registry: getDetaultNpmRegistry(true),
			pkgs: [
				{
	...
	async update() {
		...
		logger.info(
			`当前npm包${this.packageName}最新版本号为: ${latestVersion}`
		)
